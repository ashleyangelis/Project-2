...
var actorChars = {
  "@": Player,
  "o": Egg, // A egg will wobble up and down
  "=": Lava, "|": Lava, "v": Lava,
  "%": Star,
  "#": Bug
};
...
Bug.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed-0.06;
  var wobblePos = Math.sin (this.wobble) * wobbleDist;
  this.pos = this.basePos.plus (new Vector (wobblePos, -2));
};
...
Star.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed-0.05;
  var wobblePos = Math.sin (this.wobble) * wobbleDist;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "lava" || type == "bug" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "egg") {
    ...
    // If there aren't any eggs left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "egg";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
    ...
    this.player.pos = new Vector(5,4);
  }
};
